...
def test_format_context_block은_prompt_time_cue_lane을_출력한다() -> None:
    payload = {
        "mode": "prompt",
        "person_tone": [
            {
                "note_path": "entities/kim-yongseok.md",
                "title": "김용석 CTO",
...
def test_format_context_block은_general_prompt_cue_kind를_우선_출력한다() -> None:
    payload = {
        ...
        "person_tone": [
            {
                "title": "legacy should not print when prompt_cues exists",
                "do": "legacy cue",
            }
        ],
        "link_targets": [],
    ...
    assert "legacy should not print" not in block


def test_format_context_block은_prompt_time_cue를_lane당_3개로_제한한다() -> None:
    payload = {
        "mode": "prompt",
        "person_tone": [
            {
    ...
    assert "tone-0" in block
    assert "tone-1" in block
    assert "tone-2" in block
    assert "tone-3" not in block
    assert "tone-4" not in block
    assert "[[entities/sample-api]]" in block
