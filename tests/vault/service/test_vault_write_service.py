    ...
    async def exercise_writer() -> None:
        ...
        with pytest.raises(WriteConflictError, match="stale if_hash"):
            await writer.write_note(
                _write_command(body="## Summary\nStale update", created=None, if_hash="bad")
            )
...
def test_write_command는_if_hash가_있으면_created를_거부한다() -> None:
    # When / Then: if_hash가 있는 update 의도에서는 created를 다시 보낼 수 없다.
    with pytest.raises(ValidationError, match="created must not be provided"):
        _write_command(if_hash="current-hash")

    # And: 기존 note update command는 created 없이 구성되어야 한다.
    command = _write_command(created=None, if_hash="current-hash")
    assert command.created is None
...
def test_write_command는_created_updated의_초단위_UTC_Z_datetime을_요구한다(
    ...
    # When / Then: date-only, Z 없는 값, offset, 공백 구분, sub-second timestamp는 거부된다.
