...
def test_read_note는_utc가_아닌_시간을_utc_z로_저장한_뒤_반환한다(tmp_path: Path) -> None:
    # Given: UTC offset 시간이 있는 예전 note가 있다.
    vault_root = tmp_path / "vault"
    note_path = vault_root / "concepts" / "old.md"
...
""",
    ...
    read_result = reader.read_note(ReadNoteCommand(note_path="concepts/old.md"))

    # Then: 시간은 UTC Z로 파일에 저장되고 반환 hash도 저장 후 내용을 기준으로 한다.
    fixed_text = note_path.read_text(encoding="utf-8")
    assert 'created: "2026-06-12T09:30:45Z"' in fixed_text
    assert 'updated: "2026-06-12T10:31:46Z"' in fixed_text
    assert read_result.created == datetime(2026, 6, 12, 9, 30, 45, tzinfo=UTC)
    assert read_result.updated == datetime(2026, 6, 12, 10, 31, 46, tzinfo=UTC)
    assert read_result.content_hash == compute_sha256(fixed_text)


def test_read_note는_예전_공백_시간을_utc_z로_저장한_뒤_반환한다(
    tmp_path: Path,
) -> None:
    # Given: UTC 검증에서 실패하는 공백 구분 예전 시간 note가 있다.
    vault_root = tmp_path / "vault"
...
""",
    ...
    # Then: 예전 시간은 UTC로 해석되어 UTC Z 포맷으로 저장된다.
    fixed_text = note_path.read_text(encoding="utf-8")
    assert 'created: "2026-06-12T20:05:40Z"' in fixed_text
    assert 'updated: "2026-06-12T20:05:40Z"' in fixed_text
...
def test_read_note는_시간_수정시_provenance_hash를_갱신한다(tmp_path: Path) -> None:
    # Given: provenance trailer가 있는 예전 note가 있다.
    vault_root = tmp_path / "vault"
    note_path = vault_root / "concepts" / "old.md"
    note_path.parent.mkdir(parents=True)
    note_path.write_text(
...
""",
    ...
    # When: note를 읽으면서 시간을 고친다.
    reader.read_note(ReadNoteCommand(note_path="concepts/old.md"))

    # Then: provenance trailer의 source_hash도 고친 source content 기준으로 갱신된다.
    fixed_text = note_path.read_text(encoding="utf-8")
    source_content = fixed_text.split("<!-- kb-provenance:", maxsplit=1)[0]
    assert f"source_hash={compute_sha256(source_content)}" in fixed_text
    assert "source_hash=stale" not in fixed_text
    assert "operation=read_note" in fixed_text
...
def test_read후_full_body_patch와_matching_hash로_기존_note를_재작성한다(
    ...
    async def exercise_full_patch() -> None:
        ...
        patched_body = (
            f"{read_result.body}\n- [[entities/fanplus-old-api]] — Old FanPlus API boundary."
        )
