...
기본 endpoint는 `http://127.0.0.1:9999/mcp`입니다. 서버가 뜬 뒤 `GET /health`로 상태를 확인할 수 있고, MCP tool은 `kb_read_note`, `kb_search_notes`, `kb_write_note`를 노출합니다. Vault/graph counter는 REST `GET /metrics`에서 확인합니다.
...
현재 서버가 노출하는 MCP tool은 `kb_read_note`, `kb_write_note`, `kb_search_notes`입니다. Vault/graph counter는 REST `GET /metrics` endpoint로 제공합니다.
