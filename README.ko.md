...
Obsidian/Markdown LLM Wiki vault를 위한 MCP 서버입니다.
...
- write 결과의 source hash, content hash
...
기본 endpoint는 `http://127.0.0.1:9999/mcp`입니다. 서버가 뜬 뒤 `GET /health`로 상태를 확인할 수 있고, MCP tool은 `kb_read_note`, `kb_search_notes`, `kb_write_note`, `kb_delete_note`, `kb_context`를 노출합니다. Vault/graph counter는 REST `GET /metrics`에서 확인합니다.
...
- 직접 파일 접근, `kb_read_note`, 또는 `kb_search_notes` snippet으로 `SCHEMA.md`, `index.md`, `log.md` 기준 orientation 수행
...
현재 서버가 노출하는 MCP tool은 `kb_read_note`, `kb_write_note`, `kb_delete_note`, `kb_search_notes`, `kb_context`입니다. Vault/graph counter는 REST `GET /metrics` endpoint로 제공합니다.
