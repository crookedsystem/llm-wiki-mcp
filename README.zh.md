...
默认 endpoint 是 `http://127.0.0.1:9999/mcp`。服务器启动后可通过 `GET /health` 查看状态，MCP tool 暴露 `kb_read_note`、`kb_search_notes`、`kb_write_note`。Vault/graph counter 通过 REST `GET /metrics` 查看。
...
当前服务器暴露的 MCP tool 是 `kb_read_note`、`kb_write_note` 和 `kb_search_notes`。Vault/graph counter 通过 REST `GET /metrics` endpoint 提供。
