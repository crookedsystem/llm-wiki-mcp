...
Obsidian/Markdown LLM Wiki vault 向けの MCP サーバーです。
...
- write 結果に含まれる source hash、content hash
...
デフォルト endpoint は `http://127.0.0.1:9999/mcp` です。サーバー起動後は `GET /health` で状態を確認でき、MCP tool は `kb_read_note`、`kb_search_notes`、`kb_write_note`、`kb_delete_note`、`kb_context` を公開します。Vault/graph counter は REST `GET /metrics` で確認します。
...
- 直接ファイルアクセス、`kb_read_note`、または `kb_search_notes` snippet で `SCHEMA.md`、`index.md`、`log.md` を基準に orientation する
...
- 削除は先に `kb_delete_note(dry_run=true)` で対象と参照 cleanup 候補の根拠を確認する。実際の削除にはユーザーの明示的な依頼と、返された `confirmation_phrase` の完全一致が必要で、`reference_cleanup_paths` は page を削除せず、削除対象 note への wikilink だけを取り除く
- optimistic concurrency のため、返された `content_hash` を次の `if_hash` として使う
- raw source は immutable に保ち、durable wiki 変更時はナビゲーション変更を `index.md` に反映し、`log.md` は writer の自動 append に任せる
...
現在サーバーが公開している MCP tool は `kb_read_note`、`kb_write_note`、`kb_delete_note`、`kb_search_notes`、`kb_context` です。Vault/graph counter は REST `GET /metrics` endpoint で提供します。
...
write skill は本文の後に provenance trailer（`<!-- kb-provenance: ... -->`）を自動で付けます。また、成功したページ write ごとに、そのページの `type` とパスを使って `log.md` に監査エントリを append します。`log.md` は append-only なので `kb_write_note` で直接編集せず、ナビゲーション変更がある場合だけ `index.md` を更新します。`raw/` の原本は immutable に保ち、修正・統合は wiki ページ側で行います。
...
6. 答えが再利用可能な統合になったら `queries/` や `comparisons/` ページとして整理し、ナビゲーション変更がある場合は `index.md` を更新します。`log.md` は writer が自動 append します。

`kb_search_notes` は完全なファイルではなく snippet を返すため、既存 note の更新では先に `kb_read_note` で完全な body を読み、返された body を patch してから `kb_write_note(if_hash=content_hash)` を呼びます。Snippet だけで既存 note を置き換えることは引き続き禁止です。
