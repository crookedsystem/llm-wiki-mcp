...
- **Frontmatter:** `title`、`created`、`updated`、`type`、`tags`、`sources` は必須で、`created` と `updated` は秒まで含み末尾に `Z` が付いた UTC ISO datetime として保存されます（`YYYY-MM-DDTHH:MM:SSZ`）。`created` は新規 note 作成時だけ渡し、既存 note の更新時は元の作成時刻を保つため省略します。`confidence`（high/medium/low）と `contested`（true/false）は任意。
...
`kb_search_notes` は完全なファイルではなく snippet を返すため、既存 note の更新では先に `kb_read_note` で完全な body を読み、返された body を patch してから `kb_write_note(if_hash=content_hash)` を、新しい `updated` と `created` なしで呼びます。Snippet だけで既存 note を置き換えることは引き続き禁止です。
